...
build and install rules for the original HackRF software apply also to HTime.
Please, see
...
.. note::
   ...
   **Best synchronization results are obtained with HackRF One board
   revision R9.**
   ...
   involved (SI5351C): one drives the sampling clock and another drives the
   MCU clock. Because these two PLLs are independent, a random phase jump
   between the two clocks may occasionally occur — approximately 100 ns at
   a 10 MHz sampling rate — causing a transient timing error in the
   synchronization.
...
multisynthesizer section 1 generates the sampling clocks. These frequencies
are adjusted by software updating the parameters in multisynthesizer sections
1 and 2. Since this
...
The clock generation for the tuner and the mixer from the multisynthesizer
section 0 is left unchanged.
